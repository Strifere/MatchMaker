package com.example.matchmaker.utils
...
import com.example.matchmaker.domain.classes.Set
...
private fun saveMatchResult(tournament: Tournament, round: Round, player1: Player, player2: Player, matchView: View, context: Context) : Boolean {
    ...
    if (result.checkMatchResult(tournament.bestOf)) {
        round.insertResult(result)
        DomainController.getInstance().saveTournament(context, false)
    }
    else {
